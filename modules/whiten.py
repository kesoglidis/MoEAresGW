...
def torch_inverse_spectrum_truncation(psd, max_filter_len, low_frequency_cutoff=9., high_frequency_cutoff=700., delta_f=1.,
                                      trunc_method='hann'):
    N = (psd.size(1) - 1) * 2
    inv_asd = torch.zeros_like(psd)
    kmin = int(low_frequency_cutoff / delta_f)
    kmax = int(high_frequency_cutoff / delta_f)
    ...
    if kmax == 0:
        ...
        inv_asd[0, kmin:kmax] = (1. / psd[0, kmin:kmax]) ** 0.5
...
class Whiten(nn.Module):
    def __init__(self, delta_t, low_frequency_cutoff=15., high_frequency_cutoff=700., m=1.25, max_filter_len=1., legacy=True):
        super().__init__()
        # store psd estimate
    ...
    def estimate_psd(self, noise_t):
        ...
        t_psd = torch_inverse_spectrum_truncation(t_psd, int(self.max_filter_len / self.delta_t),
                                                  low_frequency_cutoff=20, high_frequency_cutoff=self.high_frequency_cutoff,
...
class CropWhitenNet(nn.Module):
    def __init__(self, net=None, norm=None, deploy=False, m=0.625, l=0.5, f=20., fl=0):
        super(CropWhitenNet, self).__init__()
        self.net = net
        self.norm = norm
        self.whiten = Whiten(1/2048, low_frequency_cutoff=f, high_frequency_cutoff=fl, m=m, max_filter_len=l, legacy=False)
        self.deploy = deploy
        self.step = 0.1

    def forward(self, x, inj_times=None, labels=None, target=None, train=True):
        segments_wh = []
        n_batch = x.size(0)
        slice_len = x.size(2)
        ...
        if self.deploy:
            ...
            if target == 'I':
                print("Yes")
                I = nn.Identity()
                return I(segments_wh)#, train=train)
            elif labels is None or target=='L':
                # import matplotlib.pyplot as plt
                # fig = plt.figure(figsize=(30,20))

                # plt.psd(self.net(segments_wh)[0][0].detach().cpu().numpy(), Fs=2048)#*2/3*10e21)
                ...
                return self.net(segments_wh, labels)#, train=train)  
            else:
                return self.net(segments_wh, labels, segments_wh)#, train=train)
