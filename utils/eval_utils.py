...
def get_triggers(Network, inputfile, step_size=0.1,
                 ...
                 num_workers=4, whiten=True, slice_length=2048, MoE=False):
    ...
    with h5py.File(inputfile, 'r') as infile:
        ...
        for slice_batch, slice_times in iterable:
            with torch.no_grad():
                ...
                if MoE:
                    output_values, segments = Network(slice_batch.to(device=device))

                    output_values, moe_loss = output_values

                    output_values = output_values, segments
                else:
