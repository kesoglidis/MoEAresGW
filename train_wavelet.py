...
from modules.AutoWavelet import GaborAutoencoder
...
def main(args):
    ...
    loss = nn.HuberLoss(reduction='mean', delta=5.0)

    base_model = GaborAutoencoder()
    ...
    for epoch in range(n_epochs):
        ...
        for idx, (training_samples, training_target, training_labels, training_inj_times) in enumerate(train_dl):
            training_samples = training_samples.to(device=train_device)
            training_target = training_target.to(device=train_device)
            ...
            import time
            # start = time.time()
            # training_target, _ = net(training_target, training_inj_times, target='Ι')
            ...
            # for i, clean in enumerate(training_clean):
            #     if training_labels[i][0] == 1.0:
            #         fig = plt.figure(figsize=(30,20))

            #         plt.plot(training_clean[i][0].detach().cpu().numpy())#*2/3*10e21)
            ...
            # start = time.time()
            training_output, training_target = net(training_target, training_inj_times, training_labels, training_target)
            # print("Output: ", time.time()-start)

            # print(training_clean)
            # energy = 1.0 / (torch.mean(training_output ** 2)+1e-6)
            
            # print("Energy: ", energy)
            training_loss = loss(training_output, training_target) * 4 #+ 0.125 * energy #+ 1 - F.cosine_similarity(training_output, training_target, dim=1)
            # start = time.time()
        ...
        with torch.no_grad():
            ...
            for val_idx, (validation_samples, validation_clean, validation_labels, validation_inj_times) in enumerate(val_dl):
                validation_samples = validation_samples.to(device=train_device)
                validation_clean = validation_clean.to(device=train_device)
                ...
                validation_clean = net(validation_clean, validation_inj_times, validation_labels)

                validation_output, validation_clean = net(validation_samples, validation_inj_times, validation_labels, validation_clean, train=False)
                validation_loss = loss(validation_output, validation_clean)
