...
from modules.dain import DAIN_Layer, MoEDAIN_Layer
from modules.whiten import CropWhitenNet
from modules.find_model import find_model
from modules.moeresnet import MoEResBlockv2
...
def main(args):
    ...
    if args.resume_from is not None:
        ...
        if 'val_pos' in checkpoint.keys():
            val_pos = checkpoint['val_pos']
            val_neg = checkpoint['val_neg']
            if MoE:
                usage_strs = checkpoint['usage']
    ...
    for epoch in range(n_epochs):
        ...
        for idx, (training_samples, training_labels, training_inj_times) in enumerate(train_dl):
            ...
            # elif labels == True:
            #     # print(labels)
            #     training_output = net(training_samples, training_inj_times, training_labels)
            else:
        ...
        with torch.no_grad():
            ...
            for val_idx, (validation_samples, validation_labels, validation_inj_times) in enumerate(val_dl):
                ...
                if MoE == True:
                    ...
                    validation_output = net(validation_samples, validation_inj_times, validation_labels)
                else:
        ...
        pos = 100 * (positive_correct / positive_total)
        neg = 100 * (negative_correct / negative_total)
        output_string = '%04i Train Loss: %f | Val Loss: %f || Train Acc: %.3f%% | Val Acc: %.3f%% (+:%.3f%%,-:%.3f%%)' % (
            epoch, training_loss, validation_loss,
            train_acc, val_acc, pos, neg)
        train_losses.append(training_loss)
        val_losses.append(validation_loss)
        train_accs.append(train_acc)
        val_accs.append(val_acc)
        # print(pos[0])
        val_neg.append(neg.item())
        val_pos.append(pos.item())
        ...
        if MoE:
            ...
            #     print(f"\n[Epoch {epoch}] Expert Usage:")
            usage_str = ''
            
            if args.model == 'MoEv3':
                usage = norm.usage.cpu().numpy()
                total = usage.sum()
                print(total)
                usage_percent = usage / total
                usage_percent = [f"{p*100:.2f}%" for p in usage_percent]

                usage_str = 'DAIN: ' + ', '.join(usage_percent) 
            moe_layers = [m for m in net.net.feature_extractor if isinstance(m, MoEResBlockv2)]

            for i, layer in enumerate(moe_layers):
                usage = layer.usage.cpu().numpy()
                total = usage.sum()
                
                usage_percent = usage / total
                usage_percent = [f"{p*100:.2f}%" for p in usage_percent]
                usage_str = usage_str + ' Layer ' +str(i)+' : ' + ', '.join(usage_percent) 

            usage_strs.append(usage_str)
            with open(os.path.join(output_dir, 'training_stats.json'), 'w') as f:
                train_dict = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, default='.', help='Path to the directory where data is stored.')
